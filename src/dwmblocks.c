...
void getcmd(const Block *block, char *output) {
    ...
    remove_all(output, '\n');
    i = strlen(output);
    if ((i > 0 && block != &blocks[LENGTH(blocks) - 1])) { strcat(output, delim); }
    i           += strlen(delim);
    output[i++]  = '\0';
}
...
void getcmds(int time) {
    ...
    for (int i = 0; i < LENGTH(blocks); i++) {
        current = blocks + i;
        if ((current->interval != 0 && time % current->interval == 0) || time == -1) { getcmd(current, statusbar[i]); }
    }
}
...
int getstatus(char *str, char *last) {
    ...
    for (int i = 0; i < LENGTH(blocks); i++) {
        ...
        if (i == LENGTH(blocks) - 1) { strcat(str, " "); }
    }
    str[strlen(str) - 1] = '\0';
...
void statusloop() {
    ...
    for (int i = 0; i < LENGTH(blocks); i++) {
        if (blocks[i].interval) { interval = gcd(blocks[i].interval, interval); }
    }
    unsigned int          i           = 0;
    int                   interrupted = 0;
